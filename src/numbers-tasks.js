...
function getLinearEquationRoot(a, b) {
  const root = +b / +a;
  if (root === 0) return 0;
  return Math.sign(b) === -1 ? Math.abs(root) : -root;
}
...
function roundToPowerOfTen(num, pow) {
  if (pow === 1) {
    return Math.round(num / 10).toFixed(pow) * 10;
  }
  if (pow === 2) {
    return Math.round(num / 100).toFixed(pow) * 100;
  }
  if (pow === 3) {
    return Math.round(num / 1000).toFixed(pow) * 1000;
  }
  return num;
}
...
function getSumOfDigits(num) {
  let sum = 0;
  const stDig = String(num);
  const stDigLenght = stDig.length;
  if (stDigLenght === 1) {
    return num;
  }
  for (let i = 0; i < stDigLenght; i += 1) {
    const newNum = stDig[i];
    sum += Number(newNum);
  }
  return sum;
}
